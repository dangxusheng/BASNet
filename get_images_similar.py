...
@site: 
@software: PyCharm
@file: get_images_similar.py
@time: 1/17/23 5:46 PM
@desc: 根据图片的相似度进行样本去重
"""
...
def test2_dirs():
    ...
    图片去重
    """
    # _dir = '/media/sunny-dxs/Workspace2/Datasets/spider_imgs/tray/塑料平板托盘-20220117'
    # _dir = '/media/sunny-dxs/Workspace2/Datasets/spider_imgs/tray/塑料网格托盘-20220117'
    _dir = '/media/sunny-dxs/Workspace2/Datasets/spider_imgs/tray/田字托盘-20220117'
    for f1 in glob.iglob(f'{_dir}/**/*.jpg',recursive=True):
        if not osp.exists(f1): continue

        for f2 in glob.iglob(f'{_dir}/**/*.jpg',recursive=True):
            if f1==f2: continue
            if not osp.exists(f2): continue
            ...
            n5 = calculate(img1, img2)
            if n5>0.9:
                print(f'{osp.basename(f1)} -- {osp.basename(f2)}  sim: {n5}')
                os.remove(f2)
